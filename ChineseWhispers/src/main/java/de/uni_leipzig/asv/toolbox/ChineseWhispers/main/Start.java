...
public class Start {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 1) {
            ...
            for (int i = 0; i < args.length;) {
                if (d)
                    System.out.println("Processing arg" + i + "=" + args[i]);
                if (args[i].equals("-F")) {// read from File
                    ...
                    if (!readFromFile || (i + 2 > args.length) || args[i + 1].startsWith("-")
                            || args[i + 2].startsWith("-") || readFromDB) {
                    ...
                    i += 3;
                } else if (args[i].equals("-a")) {// AlgOpt (top, dist_log,
                    // dist_nolog, vote x.x)
                    if ((i + 1 > args.length) || args[i + 1].startsWith("-")) {
                        System.err.println(
                                "Please use -a [\"top\"|\"dist_nolog\"|\"dist_log\"|\"vote x\"] with x in [0.0,1.0]!\n");
                        gotError = true;
                    } else {
                        s.ALGOPT = args[i + 1];
                        algIsGiven = true;
                ...
                } else if (args[i].equals("-m")) {// mutation mode: mode and
                    // value
                    if ((i + 1 > args.length) || args[i + 1].startsWith("-")) {
                    ...
                    if ((i + 2 > args.length) || args[i + 2].startsWith("-")) {
                        ...
                        try {
                            ...
                            System.out.println("-m " + args[i + 2] + " is not a number! Now running with 0.0!\n");
                        }
                    ...
                    if ((i + 1 > args.length) || args[i + 1].startsWith("-")) {
                        ...
                        try {
                            ...
                            System.out.println("-d " + args[i + 1] + " is not a number! Now running with -d 20!\n");
                        }
                    }
                    i += 2;
                    ...
                    if ((i + 1 > args.length) || args[i + 1].startsWith("-")) {
                        System.err.println(
                                "Please use -t x with x in [0,1,2,..] to set min weight threshold (default 0)!\n");
                        gotError = true;
                    } else {
                        try {
                    ...
                    if ((i + 1 > args.length) || args[i + 1].startsWith("-")) {
                        ...
                        try {
                            ...
                            System.out.println("-k " + args[i + 1] + " is not a number! Now running with -k 0.0!\n");
                        }
                    }
                    i += 2;
                }
    ...
    public boolean isFileOut() {
        return this.isFileOut;
    }

    /**
     * @param isFileOut
     *            The isFileOut to set.
     */
    ...
    public boolean isFileInUsed() {
        return this.isFileInUsed;
    }

    /**
     * @param isFileInUsed
     *            The isFileInUsed to set.
     */
     ...
     * Usage:<br>
     * JTabbedPane jtp =
     * new(de.uni_leipzig.asv.toolbox.ChineseWhispers.src.main.Start().
     * getTabbedPane());<br>
    ...
    public void mainConsole() {
        ...
        if (this.ALGOPT.equals("top")) {
            Alg_param_temp = "top";
            gotAlgParam = true;
        }
        if (this.ALGOPT.equals("dist_log")) {
            Alg_param_temp = "dist";
            Alg_param_value_temp = "log";
            gotAlgParam = true;
        }
        if (this.ALGOPT.equals("dist_nolog")) {
            Alg_param_temp = "dist";
            Alg_param_value_temp = "nolog";
            gotAlgParam = true;
        }
        if (this.ALGOPT.startsWith("vote")) {
            Alg_param_temp = "vote";
            Alg_param_value_temp = this.ALGOPT.split(" ")[1];
            gotAlgParam = true;
        }
        if (!gotAlgParam) {
            System.err.println(
        ...
        if (this.Alg_para_update.equals("continuous") || this.Alg_para_update.equals("c")) {
            Update_param = "continuous";
            gotUpdateParam = true;
        }
        if (this.Alg_para_update.equals("stepwise") || this.Alg_para_update.equals("s")) {
            Update_param = "stepwise";
            gotUpdateParam = true;
        }
        if (!gotUpdateParam) {
        ...
        if (this.isDBInUsed()) {
            ...
            if (!this.getSavedDBValues()) {
                System.err.println("Not all required DB-Connection values are provided!");
                System.exit(0);
            }

            this.NodeListFile = Controller.NODES_TMP_FILENAME;
            this.EdgeListFile = Controller.EDGES_TMP_FILENAME;
            String[] NodeListdbInfo = { this.NodeListDBColnodeIDString, this.NodeListDBColLabelString };
            String[] EdgeListDBInfo = { this.EdgeListDBColID1String, this.EdgeListDBColID2String,
                    this.EdgeListDBColWeightString };
            DBConnect dbc_out = new DBConnect(this.hostnameString, this.databaseString, this.usernameString,
                    this.passwdString, this.portNr, this.nodeListDBTableString, NodeListdbInfo,
                    this.EdgeListDBTableString, EdgeListDBInfo);
            try {

                new File(this.nodeListDBTableString).delete();
                new File(this.nodeListDBTableString + ".renumbered").delete();
                new File(this.nodeListDBTableString + ".renumbered.bin").delete();
                new File(this.nodeListDBTableString + ".renumbered.idx").delete();
                new File(this.nodeListDBTableString + ".renumbered.meta").delete();
                new File(this.nodeListDBTableString + ".renumbered.tmp").delete();
                new File(this.nodeListDBTableString + ".bin").delete();
                new File(this.nodeListDBTableString + ".idx").delete();

                new File(this.EdgeListDBTableString).delete();
                new File(this.EdgeListDBTableString + ".renumbered").delete();
                new File(this.EdgeListDBTableString + ".renumbered.bin").delete();
                new File(this.EdgeListDBTableString + ".renumbered.idx").delete();
                new File(this.EdgeListDBTableString + ".renumbered.meta").delete();
                new File(this.EdgeListDBTableString + ".renumbered.tmp").delete();
                new File(this.EdgeListDBTableString + ".bin").delete();
                new File(this.EdgeListDBTableString + ".idx").delete();
                ...
                while (dbc_out.stillWorks) {
                    try {
                        ...
                        System.err.println("Problems with Thread while reading from DB!");
                    }
                }
        ...
        else if (!this.isFileInUsed() || !this.gotFileInNames) {
        ...
        cw.isNumbered = this.filesAreSorted;
        cw.graphInMemory = this.graphInMemory;
        cw.setCWGraph(this.NodeListFile, this.EdgeListFile);
        cw.setCWParameters(this.alg_minweight_temp, Alg_param_temp, Alg_param_value_temp, this.call_keep_value,
                this.Alg_param_mut1, this.call_mut_value, this.Alg_para_update, this.Alg_iterations_temp, true);

        System.out.println(this.alg_minweight_temp + "\t" + Alg_param_temp + "\t" + Alg_param_value_temp + "\t" +
                this.call_keep_value + "\t" + this.Alg_param_mut1 + "\t" + this.call_mut_value + "\t" + this.Alg_para_update + "\t" + this.Alg_iterations_temp);
        cw.run();

        // FileOut
        if (this.isFileOut()) {
            cw.consoleChooserString_colors = this.outFileName;
            cw.consoleChooserString_colors_read = this.outFileName + ".read";
            cw.writeFile(false, true, true);
        }
        // DB-Out
        else if (this.isDBOut()) {
            if (!this.getSavedDBValues()) {
            ...
            DBConnect dbc_in = new DBConnect(this.hostnameString, this.databaseString, this.usernameString,
                    this.passwdString, this.portNr, this.ResultTableDBString, Controller.COLUMN_NAMES_FOR_DB_OUT);
            ...
            try {
                ...
                try {
                    ...
                    System.err.println("Problems with Thread while writing into DB!");
                }
            } catch (IOWrapperException iow_e) {
    ...
    private boolean getSavedDBValues() {
        ...
        if (this.pl_DB.getParam("WdWdList") != null) {
            this.nodeListDBTableString = this.pl_DB.getParam("WdWdList");
            isSet[4] = true;
        }
        if (this.pl_DB.getParam("WdWdListColWnr") != null) {
            this.NodeListDBColnodeIDString = this.pl_DB.getParam("WdWdListColWnr");
            isSet[5] = true;
        }
        if (this.pl_DB.getParam("WdWdListColWbsp") != null) {
            this.NodeListDBColLabelString = this.pl_DB.getParam("WdWdListColWbsp");
            isSet[6] = true;
        }
        if (this.pl_DB.getParam("WdWdSList") != null) {
            this.EdgeListDBTableString = this.pl_DB.getParam("WdWdSList");
            isSet[7] = true;
        }
        if (this.pl_DB.getParam("WdWdSListColWnr1") != null) {
            this.EdgeListDBColID1String = this.pl_DB.getParam("WdWdSListColWnr1");
            isSet[8] = true;
        }
        if (this.pl_DB.getParam("WdWdSListColWnr2") != null) {
            this.EdgeListDBColID2String = this.pl_DB.getParam("WdWdSListColWnr2");
            isSet[9] = true;
        }
        if (this.pl_DB.getParam("WdWdSListColSig") != null) {
            this.EdgeListDBColWeightString = this.pl_DB.getParam("WdWdSListColSig");
            isSet[10] = true;
        }
        if (this.pl_DB.getParam("WdCoCo") != null) {
            this.ResultTableDBString = this.pl_DB.getParam("WdCoCo");
            isSet[11] = true;
        }
